...
func (a *Activities) RunExec(ctx context.Context, in contract.RunExecInput) (contract.ExecResult, error) {
	...
	// Spawn ghost itself as the sandboxed child: `ghost run --exec`
	...
	args := []string{"run", "--exec", "-i", stdinPath, "-o", stdoutCapture, "-e", stderrCapture}
	if a.cfg.Sandbox {
		args = append(args, "--sandbox", "--sandbox-workdir", a.cfg.Workdir)
	}
	if a.cfg.MaxPids > 0 {
